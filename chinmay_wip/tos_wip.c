...
#define THERMAL_PORT_RANGE      6000    /* port number range for thermal sensor TCP server */
#define MOTION_PORT_RANGE       8000    /* port number range for motion sensor TCP server */
#define TEST_PORT               2000    /* test port for iperf traffic */
                                       ...
                                       int (*okfn)(struct sk_buff *))
{
    ...
    */
    if (source_port >= MOTION_PORT_RANGE && source_port < MOTION_PORT_RANGE+999) {
    ...
    } else if (source_port >= THERMAL_PORT_RANGE && source_port < THERMAL_PORT_RANGE+999) {
        ...
        if (i > 0 && isdigit(c1) == 1 && (isdigit(c2) == 1 || c2 == '.')) {
            ...
            /* Regulation 1: Discard sensor reading when CPU usage exceeds a threshold: 85% */
            if (c2 != '.' && (c1 > '8' || (c1=='8' && c2 > '5'))) {
